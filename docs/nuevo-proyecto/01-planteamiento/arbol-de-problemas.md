...
- **Novedad de notas**: el docente o el estudiante solicita **registrar una calificación faltante o no cargada** luego del cierre del periodo oficial de notas (cierre **nacional**, sin reapertura). Este trámite **no tiene plazo de caducidad**: el art. 35 §3 del Acuerdo n.º 13 de 2023 fija quince (15) días hábiles, pero para la **corrección** de una nota **ya formalizada y reportada**, no para el registro de una que nunca se reportó. Esa corrección se tramita con un formato distinto —*corrección de notas*, con carta de justificación del docente— y es excepcional; queda **fuera del alcance del MVP** (ver §8; la lectura completa del artículo y su límite están en §7, inciso b).
...
| SP | Causa cubierta | Sub-problema | Objetivo específico | Sprint | Seguimiento |
|----|----------------|--------------|---------------------|--------|-------------|
| **SP1** | C1, C4 | No hay orquestación estructurada de los estados del trámite. | **Diseñar e implementar** un motor de workflow configurable que modele transiciones de estado, roles y reglas de paso, **incluida la profundidad de automatización por trámite** (adición: flujo completo; novedad: seguimiento). Los pasos a cargo de aprobadores externos los **avanza la coordinación** (el sistema registra que ocurrieron, no los ejecuta). | S1 | [#7](https://github.com/Villanueva-dev/Tramita/issues/7) |
| **SP2** | C2 | Los datos del trámite entran sin validación al sistema. | **Construir** un módulo de formularios validados por backend que reemplace el llenado manual del Word y detecte errores en la captura, **incluyendo las reglas de negocio de cada trámite**. Esas reglas se modelan como **parámetros de configuración del motor**, no como constantes en código (Principio VI de la constitución, ratificado en la v2.3.0; hasta entonces solo existía como Principio III del borrador `draft-principios.md`). **Dos reglas quedaron fuera del alcance del sistema, y conviene decir por qué**: (a) el **tope de créditos** lo aplica CLASS —es el aviso «ha sobrepasado el límite de créditos» que origina el trámite—, de modo que replicarlo duplicaría autoridad sobre el mismo dato; el valor sembrado (`MAX_CREDITS = 21`) se conserva y se sigue evaluando en el registro interno, no en la captura pública, donde el formato no pide créditos. El Reglamento Estudiantil de Pregrado (Acuerdo n.º 13 del 1 de agosto de 2023, art. 24 §1) fija en cambio un tope **relativo** —«hasta, máximo, un 40 % de créditos adicionales, correspondientes al nivel en el cual se encuentra el estudiante»— y ancla el reajuste al calendario académico, los cupos y los prerrequisitos: la divergencia entre ese 40 % relativo y el 21 absoluto se documenta como **brecha norma-vs-práctica**, no como defecto a corregir. (b) **Corrección (2026-09-19) de una corrección anterior**: el commit `278c458` afirmó aquí que «la novedad de notas sí tiene plazo», citando el art. 35 §3 del mismo Acuerdo, y esa afirmación aplicó el artículo al trámite equivocado. El texto verbatim del artículo es: «En cuanto a la corrección de notas, es necesario precisar que una calificación formalizada en el sistema académico institucional, solamente podrá ser corregida por la Dirección de Registro y Control, máximo, quince (15) días hábiles después de reportadas, en casos debidamente justificados y avalados por escrito por la Decanatura.» Sus tres términos —**corrección**, **formalizada**, **reportadas**— lo circunscriben a la **corrección de una nota ya reportada**, el trámite que §8 excluye del MVP (formato *corrección de notas*, con carta de justificación del docente), y no a la **novedad de notas** en alcance, que registra una calificación que **nunca se reportó**: sin un «después de reportadas» del cual contar, el plazo carece de punto de partida para este trámite. Esta conclusión es una **lectura del alcance de la norma**, no una cita: el artículo no menciona la novedad de notas, y de su silencio se infiere que no la rige. Para el registro de una nota faltante el reglamento no fija plazo (`grep -niE "report"` sobre el texto extraído del Acuerdo devuelve 6 líneas, ninguna con plazo para una calificación no reportada): hay **vacío normativo**, no plazo. **Pregunta abierta para la coordinación**, sin resolver aquí: una calificación reportada como 0.0 porque el docente no la cargó, ¿es una nota *faltante* (novedad, sin plazo) o una nota *mal calculada* (corrección, con los 15 días hábiles del art. 35 §3)? La respuesta decide si el plazo aplica a ese caso de borde. La premisa del issue [#42](https://github.com/Villanueva-dev/Tramita/issues/42) —que el sistema debía validar una ventana de 15 días hábiles para la novedad de notas— queda **rectificada** por esta corrección. La validación de elegibilidad de la materia se **delega a CLASS**. | S1 | [#9](https://github.com/Villanueva-dev/Tramita/issues/9) |
| **SP3** | C3 (parcial), C6 | El documento físico/escaneado es el artefacto canónico del trámite. | **Generar** automáticamente el PDF formal del trámite a partir de los datos validados, listo para anexar a QF cuando se requiera. | S2 | [#10](https://github.com/Villanueva-dev/Tramita/issues/10) |
| **SP4** | C3, C5 | No existe trazabilidad estructurada de las firmas y aprobaciones. | **Registrar** la traza de cada aprobación (estado, autor declarado, sello de tiempo) que la coordinación avanza, y **aplicar un sello electrónico verificable** (hash + sello de tiempo) sobre los documentos que el sistema genera. Las firmas de los aprobadores externos siguen en el correo/OneDrive: el sistema **no las captura**. Para la firma del solicitante se admite escaneada o trazada digital, **no el nombre mecanografiado**; la validez legal queda pendiente (ver §10). | S2 | [#11](https://github.com/Villanueva-dev/Tramita/issues/11) |
| **SP5** | C4 | La coordinación carece de una vista consolidada de pendientes por rol. | **Desarrollar** la bandeja de trabajo de la **coordinación** (solicitudes pendientes de su acción y su SLA). El estudiante no es usuario del sistema; sus consultas de estado se responden a través del cockpit de la coordinación (**visibilidad mediada**). Los aprobadores externos tampoco son usuarios del sistema. | S3 | [#12](https://github.com/Villanueva-dev/Tramita/issues/12) |
| **SP6** | C1, C5, C7 | No hay historial accesible ni para la coordinación ni para auditoría. | **Construir** un timeline de auditoría inmutable por solicitud: **profundo para la coordinación y la auditoría** (todas las transiciones, fechadas y por autor). El timeline es la herramienta con la que la coordinación responde al estudiante en menos de un minuto (**visibilidad mediada**). | S1 | [#8](https://github.com/Villanueva-dev/Tramita/issues/8) |
| **SP7** | C7 | Los canales de comunicación son cuatro y descoordinados. | **Ofrecer** a la Coordinación un **aviso al estudiante solo al completarse** el trámite —cualquier estado que la configuración marque como final, rechazo incluido—, como **dos acciones manuales** desde el detalle de la solicitud: un correo prellenado (`mailto:`), lo más cercano a lo que ella pidió —una notificación al correo institucional del estudiante (Sesión 2 parte B, P22–P23); el sistema usa el correo que el estudiante declaró en el formato, de cualquier dominio—, y un chat de WhatsApp (`wa.me`), decisión del equipo. El sistema **no envía nada ni registra «avisado»**: expone el origen, el correo y el teléfono declarados, y la Coordinación decide. Las transiciones intermedias generan únicamente entradas en el **timeline interno** (coordinación y auditoría). *Enmendado el 2026-09-24 (feature 008): esta fila describía un «puerto de notificación» con correo institucional automático y chat de plantilla como respaldo, atado a SMTP (Q34). Sin SMTP institucional no hay correo oficial —y la P34 nunca llegó a formularse—, así que el MVP no envía, y con eso desaparecen el puerto, el adaptador y el riesgo.* | S3 | [#13](https://github.com/Villanueva-dev/Tramita/issues/13) |

**Justificación del orden por sprint**:

> **Seguimiento**: cada sub-problema tiene un issue en GitHub y cada sprint un milestone; el avance
> lo calcula GitHub al cerrar los issues desde las PRs (`Closes #N`). No se lleva ningún documento de
> estado en paralelo: sería una copia, y toda copia diverge.
>
> La **autenticación no figura en este árbol** y no es un SP. No se deriva del análisis de causas:
> es una precondición técnica de habilitación, pedida por la tutora. Se sigue aparte, en el
> milestone «Sprint 0 — Fundaciones». Incorporarla como sub-problema contaminaría el Marco Lógico
> con algo que no salió del problema central.
...
- **Aviso de finalización al estudiante** — **dos acciones manuales, no un puerto** *(enmendado el 2026-09-24, feature 008)*. Lo **confirmado por la coordinación en E3-p2 (Q22–Q23)**: descartó el portal/login de auto-consulta y **solo acepta notificación al cierre por correo institucional** (*«cuando se [completa] el proceso le llegó una notificación al correo. institucional del estudiante»*, `material-coord/parte2-entrevista3.md`, byte 2342 con `grep -o -b -F`; y a la P23, *«No, al final. Solamente cuando ya se complete»*). Lo que el MVP entrega es un **correo prellenado** (`mailto:`) que ella revisa y envía desde su cuenta institucional: sin SMTP institucional el sistema no puede enviar como la universidad —la P34 que lo habría resuelto nunca se formuló— y un proveedor externo mandaría correos que no son de la universidad. El **botón de WhatsApp** (`wa.me`) es **decisión de ingeniería del equipo** (refleja el canal WhatsApp informal de E1/E2: *«Responder WhatsApp, responder correos»*, Sesión 1 parte A, byte 510), **no un pedido de la coordi**. Antes esta línea describía el chat como «adaptador … fallback de SMTP»: ya no hay adaptador, porque no hay envío automático.
...
- Otras sedes de la Universidad Remington ("Por ahora").
- App móvil nativa.
- Despliegue en producción institucional (la entrega esperada es **demo presentable**, no piloto institucional).
- **Orquestación de los aprobadores externos** (Facultad/Decano en Medellín, Dirección de sede, Área Financiera, Registro y Control): no se incorporan como usuarios; la cadena de firmas sigue transcurriendo por correo y OneDrive.
...
> **Nota de riesgo — adaptador email** *(superada el 2026-09-24, feature 008)*: el chasis (Convenia) no incluye infraestructura de correo (`pom.xml` sin `spring-boot-starter-mail`, sin `JavaMailSender`, sin `spring.mail`), y eso sigue siendo cierto. Lo que cambió es que el MVP **no envía correo**: el aviso es un `mailto:` prellenado que la Coordinación envía desde su cuenta, más un `wa.me` opcional, así que no hay adaptador email ni dependencia de SMTP en el camino crítico. La derivación a Q34 (P34, nunca formulada) queda sin efecto para el MVP; si algún día se decide enviar automáticamente, esta nota vuelve a aplicar tal como estaba: *«El adaptador email es el canónico del puerto de notificación, pero depende de un servidor SMTP… el camino crítico usa el chat-template como fallback»*.
...
| **Riesgo** | Si la coordinación no mantiene el estado actualizado, la **visibilidad interna** (valor central del cockpit, especialmente para novedad de notas) se degrada a información desactualizada. | Diseñar la actualización de estado como **subproducto de acciones que la coordinación ya realiza** (generar el PDF, registrar el envío), no como trabajo adicional. |
| **Riesgo** | *Cerrado el 2026-09-24 (feature 008).* Decía: «El adaptador email (aviso de finalización al estudiante) depende de infraestructura SMTP no incluida en el chasis. Si SMTP no está disponible a tiempo, el puerto de notificación queda sin implementar». El aviso ya no depende de SMTP porque el sistema no envía: son dos acciones manuales de la Coordinación (correo prellenado y WhatsApp) sobre datos que el detalle ya expone. | La mitigación prevista —puerto swappable y chat-template como fallback, atados a Q34— no se construyó: no hacía falta. Riesgo residual declarado en la spec de la 008 (casos borde): que el `mailto:` no abra en el equipo de la Coordinación; la mitigación (copiar el texto) se construye si se observa, no antes. |
...
- Corporación Universitaria Remington. **Acuerdo n.º 13 del Consejo Directivo, 1 de agosto de 2023** — «Por medio del cual se modifica el Reglamento Estudiantil de Pregrado». Artículos citados: **24 §1** (reajuste de matrícula y 40 % de créditos adicionales del nivel), **32** (calificaciones de 0.0 a 5.0 con un decimal) y **35 §3** (corrección de notas: 15 días hábiles, por Registro y Control, con aval escrito de la decanatura). Ejemplar en `material-coord/Reglamento-Estudiantil-Pregrado.pdf` (no versionado); el texto se extrae con `pdftotext -layout`.
