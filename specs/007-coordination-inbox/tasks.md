...
- [x] T005 [US1] **(RED)** En `src/test/java/com/uniremington/api/tramita/controller/RequestControllerIT.java`: una solicitud cuyo estado actual tiene una transición con `responsible = X` **aparece** en `GET /api/requests/inbox?responsible=X`, y la lista tiene **exactamente el tamaño esperado** (`$.length()`), no solo «contiene». *(Corregido tras el review, M1: sin `distinct` NO hay duplicados —Hibernate deduplica la entidad raíz—; lo que `distinct` decide es que la cota cuente solicitudes y no filas del join, y eso lo prueban dos IT en `WorkflowGenericityIT`.)* Cubrir también una solicitud **devuelta** (`DEVUELTA`): aparece en la bandeja de `COORDINACION`, porque es ella quien registra el reingreso (research D1, spec US1 escenario 6). Debe fallar porque el parámetro todavía no existe y la lista viene sin criterio
