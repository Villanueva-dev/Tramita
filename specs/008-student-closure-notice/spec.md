...
**Status**: Implementada — 2026-09-24, commits `82e5aaf`…`813113d` en la rama `008-student-closure-notice`; gate `review-spec` superado y auditada contra las fuentes el mismo día
...
**Why this priority**: es el canal que la Coordinación pidió y el momento que fijó. Lo dijo así: *«cuando se [completa] el proceso le llegó una notificación al correo. institucional del estudiante»* (Sesión 2 parte B, `material-coord/parte2-entrevista3.md`, byte 2342 con `grep -o -b -F 'correo. institucional del estudiante'`; el punto tras «correo» es de la transcripción, y la voz es de ella por el ancla en primera persona que la precede, *«registro y control me dice»*, byte 2189), y a la pregunta de si avisar en cada cambio de estado respondió *«No, al final. Solamente cuando ya se complete»* (P23). Además cubre el 100 % de las solicitudes del enlace público, porque ahí el correo es obligatorio. Sin esta historia no hay aviso.
...
El formulario público deja de aceptar como teléfono cualquier texto: exige un número de 10 dígitos en formato nacional colombiano. Así, cada solicitud nueva llega con un número que sirve para avisar.
...
- **FR-003a**: El sistema MUST NOT ofrecer el aviso en estados intermedios. Es la única restricción de momento que la Coordinación fijó: *«No, al final. Solamente cuando ya se complete»* (Sesión 2 parte B, respuesta directa a la P23). La frase *«Solamente estado final, únicamente»* que sigue en la transcripción es una reformulación cuya voz no se puede atribuir —usa el vocabulario del guion, no el de ella— y por eso no se cita como suya (auditoría del 2026-09-24, I1).
...
- **FR-009**: El canal público MUST rechazar un teléfono que no tenga exactamente 10 dígitos. El rechazo nombra el campo y nunca repite el valor, con el mismo mecanismo con que ese canal ya informa campos faltantes o inválidos. Se exige el **formato nacional colombiano**: diez dígitos, sin prefijo internacional. El sistema **no verifica la nacionalidad** del número: diez dígitos son condición necesaria, no suficiente, y los números extranjeros quedan fuera por decisión del usuario, no por comprobación.
...
- **El correo automático queda descartado**: sin el servidor institucional no hay correo oficial, y un proveedor externo mandaría correos que no son de la universidad y le entregaría los datos a un tercero. Lo decidió el usuario el 2026-09-23 y no se reabre. ⚠️ La **disponibilidad** de SMTP institucional **no se consultó**: la P34 del guion, que la cubría, nunca se formuló y se derivó al docente asesor sin respuesta registrada (`material-coord/evidencia-entrevistas-coordinacion.md:739-742`). La decisión no depende de esa respuesta: el sistema no envía por diseño (FR-006), y se mantiene aunque el SMTP aparezca, salvo decisión nueva del equipo.
- **WhatsApp es una decisión del equipo, no un pedido**: `arbol-de-problemas.md:164` lo dice textualmente. En las transcripciones no consta que la Coordinación avise el cierre por WhatsApp caso por caso. Los rastros del canal son tres, y ninguno habla del cierre: *«Responder WhatsApp, responder correos»* (Sesión 1 parte A, `material-coord/transcript-entrevista-coordi.md`, byte 510: lo primero que nombra al preguntarle qué le quita tiempo), *«El WhatsApp de formación con el estudiante»* (Sesión 1 parte B, `transcript-entrevista-coordi-2.md`, byte 17057, en un pasaje de transcripción deficiente) y *«Le llamo, le escribo por WhatsApp»* (Sesión 2 parte A, `entrevista-Coordi-3.txt:227`, con docentes). WhatsApp es un canal vivo de la Coordinación; que sirva para avisar el cierre no consta. **Supuesto a validar con la Coordinación**: que al estudiante le llega mejor por WhatsApp que por correo. Por eso WhatsApp es la opción P2 y no la principal.
...
- **El rechazo también se avisa. Supuesto a validar**: la Coordinación dijo *«No, al final. Solamente cuando ya se complete»* (Sesión 2 parte B, P23), y la fila SP7 del árbol hablaba de «estado FINALIZADO» (`arbol-de-problemas.md:134`, enmendada el 2026-09-24). Que ella comunica hoy el rechazo sí consta: *«el decano me lo negó… Le dije, ¿sí ve? Es culpa suya»* (Sesión 2 parte A, `material-coord/entrevista-Coordi-3.txt:49-51`) y *«El decano lo rechazó»* (Sesión 2 parte B, byte 5567). Lo que no consta es el **canal**. Esta feature ofrece el aviso en cualquier estado final, rechazo incluido, por dos razones: para el estudiante el rechazo también es el fin del trámite, y la configuración lo marca como final. Queda por confirmar que la Coordinación avisa el rechazo por el mismo canal. Por eso el mensaje nombra el estado y no presupone el resultado (FR-005a).
- **Por qué solo el enlace público**: las solicitudes que registra la Coordinación le llegaron por correo. Lo dijo ella: *«El estudiante le digo que me lo mande por correo electrónico, el formato, haciéndome la solicitud. Hacer la solicitud formar por correo electrónico al correo de Cali»* (Sesión 1 parte A, `material-coord/transcript-entrevista-coordi.md`, byte 6019 con `grep -o -b -F 'por correo electrónico al correo de Cali'`; la síntesis `2026-06-03-entrevista1-sintesis-analitica.md:45` lo resume como «correo formal al correo institucional de Cali»), así que la conversación con el estudiante ya existe en ese hilo. Las del enlace público, en cambio, no tienen ningún hilo previo.
...
- **El árbol de problemas queda desactualizado** *(✅ enmendado el 2026-09-24, T038)*: la fila SP7 (`:134`), la línea `:164`, la nota de riesgo `:181` y el riesgo `:216` describían el correo automático como adaptador canónico y el chat como respaldo. Esta feature no construye el correo automático y ofrece los dos avisos como acciones manuales. El árbol es la fuente de verdad del planteamiento y debe enmendarse para reflejarlo, como se hizo con el plazo en la PR #44.
