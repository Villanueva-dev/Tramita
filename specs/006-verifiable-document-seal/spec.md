...
**Independent Test**: se emite un documento y se verifica sin tocarlo: da **íntegro**, incluso si mientras tanto cambió el formato o avanzó la solicitud; se altera un byte del archivo: da **alterado**; se altera el archivo y la diferencia coincide con que el formato o la revisión ya no son los de la emisión: da **no verificable**, jamás «alterado».
...
3. **Given** un documento cuya huella no coincide con la guardada porque se emitió con una versión del formato que ya no es la vigente, **When** se verifica, **Then** el resultado es **no verificable**, acompañado del motivo, y **nunca** «alterado». Un documento sin alterar sigue dando **íntegro** aunque el formato haya cambiado desde que se emitió.
4. **Given** un documento cuya huella no coincide con la guardada porque se emitió sobre una revisión de los datos anterior a la actual de la solicitud, **When** se verifica, **Then** el resultado es **no verificable**, y **nunca** «alterado». Un documento sin alterar sigue dando **íntegro** aunque la solicitud haya avanzado desde que se emitió — es la prueba que `quickstart.md` (paso 5) declara imprescindible.
...
- **El formato cambia** (logo, maquetación, tipografías, textos fijos del papel): todos los sellos anteriores dejan de poder **reconstruirse** —mostrar de nuevo el documento tal como salió— **a la vez y en silencio** (riesgo residual declarado en `research.md` D5). Eso no cambia el veredicto de un documento legítimo que nadie tocó: su huella sigue coincidiendo con la guardada y sigue dando **íntegro**. El formato vencido entra en juego solo cuando la huella **no** coincide: ahí explica la discrepancia en vez de acusar, y el sistema responde **no verificable**, nunca alterado. Un cambio de formato no es una falsificación.
- **Los datos de la solicitud cambian después de emitir**: el documento emitido sobre la revisión 3 sigue dando **íntegro** si es el mismo archivo que salió —su huella no depende de la revisión actual de la solicitud—. La revisión entra en juego solo cuando la huella recibida **no** coincide con la guardada: ahí, que el sello quedó sobre una revisión que ya no es la actual explica la discrepancia sin acusar. Tampoco es una falsificación: es un documento viejo. Debe dar **no verificable**, no «alterado».
...
- **FR-004**: Reconstruir el documento de una solicitud **a partir de un sello ya registrado** —mismos datos, mismo formato y el mismo código de verificación que ese sello guarda— MUST producir un documento idéntico al que se emitió. Esta reproducibilidad sostiene la reconstrucción del documento para mostrarlo; el veredicto de integridad (FR-006) compara la huella recibida contra la huella que el sello guardó al emitir, y no depende de reconstruir nada.
...
- **FR-006**: La verificación de un documento MUST devolver exactamente uno de tres resultados: **íntegro**, **alterado**, o **no verificable**, a partir de comparar la huella recibida contra la huella que el sello guardó al emitir. Una coincidencia MUST devolver **íntegro** de forma definitiva, sin más comprobaciones; solo cuando no coincide el sistema aplica las guardas de FR-007 antes de concluir que hay alteración. Estos tres resultados pertenecen a la verificación exacta (FR-014d); el canal público (FR-014b) solo afirma la emisión.
...
- **FR-010**: El sistema MUST NOT almacenar los archivos PDF emitidos. Trámita no es un repositorio de archivos; la verificación exacta se resuelve comparando huellas —la recibida contra la que el sello guardó al emitir—, sin reconstruir ni almacenar el documento.
...
- **FR-014e**: El pie legible MUST quedar completo desde la primera emisión sellada. Agregarle algo después cambia la versión del formato: los sellos anteriores no dejan de dar **íntegro** si nadie los tocó, pero si alguna vez hace falta explicar una huella que no coincide, ya no van a poder apoyarse en «el formato sigue vigente» (FR-007) — un daño que el propio sistema se causaría.
...
- **SC-001**: Reconstruir un documento a partir de su sello produce un archivo idéntico al emitido el **100 %** de las veces.
