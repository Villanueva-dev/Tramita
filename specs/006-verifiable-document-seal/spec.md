...
- **El mismo documento se emite varias veces sin que nada cambie**: las emisiones producen documentos idénticos y se registran por separado, porque la pregunta «cuántas veces se emitió y quién lo pidió» es parte de la trazabilidad que esta feature entrega.
...
- **FR-004**: Emitir dos veces el documento de una solicitud cuyos datos y formato no cambiaron MUST producir documentos idénticos. Sin esta reproducibilidad, la huella no verifica nada.
...
- **FR-013a**: El sistema MUST NOT aceptar en silencio una calificación que luego almacenará redondeada. Una calificación con más precisión de la que la norma admite se rechaza con un mensaje que dice por qué, en vez de guardarse alterada.
...
- **SC-001**: Emitir dos veces el documento de una solicitud sin cambios produce archivos idénticos el **100 %** de las veces.
...
- **Cada emisión se registra por separado, sin deduplicar.** Aunque dos emisiones sin cambios produzcan documentos idénticos, se guardan como dos sellos: la pregunta «cuántas veces y quién» forma parte de la trazabilidad pedida.
