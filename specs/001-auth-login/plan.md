...
(mínimo 15, máximo 72, sin composición ni rotación forzadas).
...
| **II. Arquitectura por feature** | ✅ Paquete `auth` autocontenido (controller, service+impl, repository, entity, DTOs, política, throttling). `shared/` (config, exception, seed) solo para cross-cutting legítimo. Rompe a propósito el layout by-layer de Convenia. |
...
│   ├── AuthController.java             # POST /login /logout /password, GET /me
│   ├── AuthService.java               # interface
│   ├── AuthServiceImpl.java           # login programático, cambio de clave
│   ├── AppUserDetailsService.java     # carga User por email → UserDetails
│   ├── PasswordPolicy.java            # reglas NIST (15..72, != actual)
│   ├── LoginAttemptService.java       # throttling en memoria (sliding window)
...
└── shared/                           # cross-cutting (plumbing transversal)
    ├── config/
    │   ├── SecurityConfig.java          # SecurityFilterChain, CORS, CSRF, cookie, throttle hook
    ...
    └── seed/
        └── CoordinationUserSeeder.java  # provisión idempotente por env (no hash en git)
