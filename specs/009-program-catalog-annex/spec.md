...
- **Tildes y eñes**: el nombre viaja tal como el sistema lo publica; un cliente que lo altere por el camino (otra normalización de caracteres, espacios de más) verá el rechazo del escenario 4. Cuatro de los trece nombres llevan tilde.
...
- **Enmienda no aditiva y orden de despliegue**: hoy el cliente público envía el programa escrito por el estudiante, y el interno envía uno de su lista de cinco, de los que cuatro no están en el catálogo. Si el backend llega antes que el cliente, el enlace público rechazará todo programa que no coincida letra por letra, y el interno rechazará cuatro de sus cinco opciones. El cliente tiene que reemplazar los dos campos por la lista del catálogo **antes o a la vez**. El reparto exacto con el repositorio del frontend, que lleva otro agente, se fija en el plan; el precedente es el brief de la 008 (`tramita-frontend#59`).
...
- **Enmienda del issue #40**: al abrir la PR se reformulan tres puntos de su texto. La regla es por programa, no por facultad, y el nombre `REQUIRED_ANNEX_BY_FACULTY` que propone queda desalineado. El criterio «al pasar a la facultad» pasa a «desde el registro, y también en la respuesta de esa acción» (FR-009). Y su referencia a que el #25 «sigue abierto» venció: se cerró el 2026-09-21. Las líneas de la entrevista que cita (`:184`, `:551`, `:760`) hoy son `:184`, `:553` y `:762`.
- **Issue del catálogo**: no existía al escribir esta spec. Se abre con ella, y la PR de la feature cierra los dos.
- **Alcance**: Sede Cali, programas de pregrado y los dos trámites del MVP. Quedan fuera: recibir o almacenar cualquier archivo; administrar el catálogo desde la interfaz; derivar la facultad del programa; un catálogo por sede; posgrados, tecnologías y técnicos; la ventana temporal (#42) y el tope de créditos (#17).
