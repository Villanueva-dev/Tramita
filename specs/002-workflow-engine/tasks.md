...
- [X] T040 Ejecutar el guion completo de `quickstart.md` contra la app corriendo (perfil dev). **Ejecutado de punta a punta el 2026-08-07**: login 204, registro 201 naciendo en `REGISTRADA`, avance 200, transición ilegal 409, devolución sin motivo 422 y con motivo 200, búsqueda por cédula, timeline con el nacimiento y el motivo, `search=%%` → `[]` (el fix del code review, en vivo), SC-005 con el trámite `DEMO` cargado por SQL y operado sin redeploy (201) y SC-002 con `UPDATE`/`DELETE` del timeline rechazados por el trigger. El guion tenía dos defectos que lo volvían inejecutable, corregidos en `dc570bd`: el login omitía el CSRF y el SQL de SC-005 omitía `created_at` (NOT NULL sin DEFAULT). **La causa que esta tarea registraba antes —el `.env` sin quotear— era falsa**: se resembró la cuenta con la variable verificada intacta en el proceso y el 401 persistía. Lo que faltaba en el guion era el paso previo que trae la cookie `XSRF-TOKEN`. **Rectificación del 2026-08-16**: la explicación que se escribió entonces —«un rechazo de CSRF sobre usuario anónimo deriva al `AuthenticationEntryPoint` y responde 401, no 403»— **es incorrecta**. `CsrfFilter` resuelve la denegación con su propio `AccessDeniedHandlerImpl` y corta la cadena (`CsrfFilter.java:89` y `:132-133`), así que la excepción nunca llega al `ExceptionTranslationFilter`: **sin CSRF la respuesta es `403`**, probado en `AuthControllerIT#loginWithoutCsrfTokenIsRejected`. La causa del `401` observado aquel día **quedó sin determinar** — el experimento que pretendía descartar la contraseña era inválido, porque el seeder es idempotente (`CoordinationUserSeeder.java:47-50`) y la clave de prueba nunca llegó a aplicarse
